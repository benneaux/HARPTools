...
files <- as_tibble(list.files(file.path, 
                              pattern = ".XLS"
                              )
                   )
file.name <- filter(files,
...
clean.data <- clean.data %>% slice(junk.s:(n()-junk.e))
...
temp <- clean.data %>%  subset(select = -1)
...
if(TRUE %in% str_detect(temp[,],"Reversal")){
  ...
  temp <- temp %>% select(-rev,everything())
  
  temp[,length(temp)] <- gsub("Reversal",
                              "Yes",
                              temp[,length(temp)]
                              )
}
...
if(length(data.b)==6){
  ...
  for(i in 1:nrow(data.b)){if(is.na(data.b[i,length(data.b)])==TRUE){
...
data.b[,"Prescriber"] <- gsub("Prescriber: ",
                              "",
                              data.b[,"Prescriber"]
                              )

# Removes unnescessary text from "Date:"
data.b[,"Date"] <- gsub("Date: ",
                        "",
                        data.b[,"Date"]
                        )
...
temp <- data %>% subset(select = 1)

# Find the index of each row containing the string 'Site: ".  
temp.ind <- which(str_detect(temp$X1,"Site: "))
...
temp.ind <- which(str_detect(temp$X1,": "))
...
clean.data3 <- clean.data3[!str_detect(clean.data3[,1],
                                       "Inpatient|Outpatient|Discharge"
                                       ),]
...
###############################################################################
#
# Split MRNs and Drugs into separate variables.
#
###############################################################################
...
final$Site_Code <- str_split_fixed(
  str_split_fixed(final$Site," \\(",2)[,2],"\\)",2)[,1]
...
if("Reversal" %in% names(final)){
  final <- select(final,
                  1:2,11,3,12,everything()
                  )
} else {
  final <- select(final,
                  1:2,10,3,11,everything()
                  )
}
...
# All Dispensing
final.Dispensing <- final %>%  filter(., Cost>=0)

# All Reversals
final.Reversals <- final %>%  filter(.,Cost<0)
